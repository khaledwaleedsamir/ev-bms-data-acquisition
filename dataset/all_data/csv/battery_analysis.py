...
def plot_overview(df: pd.DataFrame,
    ...
    """
    time_min = (df["datetime_utc"] - df["datetime_utc"].iloc[0]).dt.total_seconds() / 60

    fig, axes = plt.subplots(2, 2, figsize=(10, 7))
    fig.suptitle("Battery Cycle Overview", fontsize=13, fontweight="bold", y=1.01)

    # Set common x-axis limits
    x_min, x_max = time_min.min(), time_min.max()

    # ── Voltage ──────────────────────────────────────────────────────────────
    ax = axes[0, 0]
    ax.plot(time_min, df["voltage_V"], color=C["voltage"], label="Voltage")
    ...
    # ── Current ──────────────────────────────────────────────────────────────
    ax = axes[0, 1]
    ax.plot(time_min, df["current_A"], color=C["current"], label="Current")
    ...
    # ── Temperature ──────────────────────────────────────────────────────────
    ax = axes[1, 0]
    ax.plot(time_min, df["temperature_degC"], color=C["temp"], label="Temperature")
    ...
    # ── SOC ──────────────────────────────────────────────────────────────────
    ax = axes[1, 1]
    ax.plot(time_min, df["bms_soc_pct"],  color=C["bms_soc"],  label="BMS SoC",
            linestyle="-")
    ax.plot(time_min, df["pred_soc_pct"], color=C["pred_soc"], label="Predicted SoC",
...
def clean_esp_temp(df: pd.DataFrame,
    ...
    if plot:
        ...
        ax = axes[0]
        ax.plot(time_min, raw, color=C["esp_temp"], linewidth=1.2, label="Raw signal")
        ax.scatter(time_min[outlier_mask], raw[outlier_mask],
        ...
        ax = axes[1]
        ax.plot(time_min, cleaned, color=C["cleaned"], linewidth=1.2,
...
def soc_metrics(df: pd.DataFrame,
    ...
    # ── Diagnostic figure ─────────────────────────────────────────────────
    time_min = (df["datetime_utc"].iloc[valid.index] - df["datetime_utc"].iloc[0]).dt.total_seconds() / 60
    ...
    ax = axes[0]
    ax.plot(time_min, y_true, color=C["bms_soc"],  linewidth=1.4,
            label="BMS SoC")
    ax.plot(time_min, y_pred, color=C["pred_soc"], linewidth=1.2,
